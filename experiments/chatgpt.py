...
@retry_on_exception()
def complete(messages=None, model='gpt-4', temperature=0, use_cache=True, **kwargs):
    if use_cache:
...
@retry_on_exception()
async def acomplete(messages=None, model='gpt-4', temperature=0, use_cache=True, **kwargs):
    if use_cache:
        key = get_key(messages)
        if key in cache:
            return cache.get(key)
